...
export default class Register extends React.Component{
    ...
    handleRegister = ()=> {
        ...
        if(state.accounts.length !== 0){
            this.usernameDuplicate(state.accounts, username) ? errorCode=2 : this.handleRemoveError("username");
            this.emailDuplicate(state.accounts, email) ? errorCode=6 : this.handleRemoveError("email");
        }

        this.validateEmail(email) ? this.handleRemoveError("email") : errorCode=3;
